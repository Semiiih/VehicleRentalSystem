  ...
  useEffect(() => {
    fetch("/api/vehicles")
      ...
      .then((data) =>
        setVehicle(
          Array.isArray(data)
            ? data.find((v) => v.id === parseInt(id)) || null
            : null,
        ),
      )
  ...
  return (
    <div className="bg-[#f4f5f6] min-h-[calc(100vh-4rem)]">
      <div className="max-w-5xl mx-auto px-6 py-12">
        <Link
          to="/catalog"
          className="text-[#ff5000] font-semibold text-sm hover:underline mb-6 inline-block"
        >
          ← Retour au catalogue
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <span className="text-xs font-bold uppercase tracking-wide text-[#ff5000]">
              {TYPE_LABELS[vehicle.type]}
            </span>
            <h1 className="text-3xl font-black text-[#1a1a1a] mt-1">
              {vehicle.brand}
            </h1>
            <div className="text-sm text-gray-500 mt-1">
              {vehicle.type === "CAR" && `${vehicle.numberOfDoors} portes`}
              {vehicle.type === "BIKE" &&
                (vehicle.electric ? "Électrique" : "Non électrique")}
              {vehicle.type === "TRUCK" &&
                `Charge : ${vehicle.loadCapacity} kg`}
            </div>
            <div className="mt-4">
            ...
            <div className="bg-white rounded-xl p-5 mt-6 shadow-sm">
              ...
              <div className="grid grid-cols-7 gap-1">
                {cells.map((day, i) => {
                  ...
                  return (
                    <div
                      key={dateStr}
                      className={`text-center py-1.5 rounded text-xs font-medium ${booked ? "bg-red-500 text-white" : "bg-green-50 text-green-700"} ${isToday ? "ring-2 ring-[#ff5000]" : ""}`}
                    >
                      {day}
                    </div>
                  );
              ...
              <div className="flex gap-4 mt-3 text-xs text-gray-400">
                <span className="flex items-center gap-1">
                  <span className="w-2.5 h-2.5 rounded bg-green-50 border border-green-200 inline-block" />{" "}
                  Disponible
                </span>
                <span className="flex items-center gap-1">
                ...
                {getPrice() && (
                  <div className="bg-[#f4f5f6] rounded-lg p-4">
                    <div className="flex justify-between text-sm text-gray-500 mb-1">
                      <span>
                        {vehicle.basePricePerDay}€ × {getDays()} jours
                      </span>
                      <span>
                        {(vehicle.basePricePerDay * getDays()).toFixed(2)}€
                      </span>
                    </div>
                    {vehicle.type === "BIKE" && (
                      <div className="flex justify-between text-sm text-green-600 mb-1">
                        ...
                        <span>
                          -
                          {(vehicle.basePricePerDay * getDays() * 0.1).toFixed(
                            2,
                          )}
                          €
                        </span>
