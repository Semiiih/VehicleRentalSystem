...
const defaultForm = {
  type: 'CAR', brand: '', basePricePerDay: '',
  numberOfDoors: '', electric: false, loadCapacity: '',
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-sm shadow-2xl w-[520px] overflow-hidden">
        ...
        <div className="bg-[#141414] px-6 py-4 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold tracking-widest uppercase text-[#FF5F00]">Disponibilités</p>
            <p className="text-white font-black text-lg tracking-tight">{brand}</p>
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E0E0E0]">
          <button onClick={prev} className="w-8 h-8 flex items-center justify-center border border-[#E0E0E0] rounded-sm text-[#666] hover:border-[#FF5F00] hover:text-[#FF5F00] transition-colors text-sm font-bold">‹</button>
          <span className="text-sm font-black uppercase tracking-widest text-[#141414]">
            {monthNames[month.month]} {month.year}
          </span>
          <button onClick={next} className="w-8 h-8 flex items-center justify-center border border-[#E0E0E0] rounded-sm text-[#666] hover:border-[#FF5F00] hover:text-[#FF5F00] transition-colors text-sm font-bold">›</button>
        </div>
        ...
        <div className="p-5">
          ...
          <div className="grid grid-cols-7 mb-2">
            {dayNames.map(d => (
              <div key={d} className="text-center text-[9px] font-bold uppercase tracking-widest text-[#AAAAAA] py-1">{d}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
              ...
              const isToday = day === today.getDate() && month.month === today.getMonth() && month.year === today.getFullYear()
              return (
                <div
                  key={day}
                  title={info ? `${info.customer?.name || 'Client'} · ${info.startDate} → ${info.endDate}` : ''}
                  className={`
          ...
          <div className="flex items-center gap-6 mt-4 pt-4 border-t border-[#E0E0E0]">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#666]">Loué</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-sm bg-[#F2F2F2] border border-[#E0E0E0]" />
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#666]">Disponible</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-sm bg-[#141414]" />
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#666]">Aujourd'hui</span>
            </div>
          </div>
          ...
          {rentals.length > 0 && (
            <div className="mt-4 space-y-2">
              <p className="text-[9px] font-bold uppercase tracking-widest text-[#AAAAAA]">Locations actives</p>
              {rentals.map(r => (
                <div key={r.id} className="flex items-center justify-between bg-[#F2F2F2] rounded-sm px-3 py-2">
                  <div>
                    <span className="text-xs font-bold text-[#141414]">{r.customer?.name || '—'}</span>
                    <span className="text-[10px] text-[#AAAAAA] ml-2">{r.startDate} → {r.endDate}</span>
                  </div>
                  <span className="text-xs font-black text-[#FF5F00]">{r.totalPrice}€</span>
                </div>
              ))}
            </div>
          )}
...
export default function VehiclesPage() {
  ...
  const avgPrice = vehicles.length
    ? (vehicles.reduce((s, v) => s + v.basePricePerDay, 0) / vehicles.length).toFixed(0)
    : 0
  ...
  return (
    <div className="font-['Inter',sans-serif] bg-[#F2F2F2] min-h-screen p-8 pb-16">
      ...
      <div className="bg-[#141414] rounded-sm px-12 py-10 mb-7 flex items-center justify-between relative overflow-hidden"
        ...
        <div>
          <p className="text-[11px] font-bold tracking-[.15em] uppercase text-[#FF5F00] mb-2.5">Administration · Gestion de flotte</p>
        ...
        <div className="flex gap-0">
          {[
            ...
            { val: [...new Set(vehicles.map(v => v.type))].length, suffix: null, lbl: 'Catégories' },
          ].map(({ val, suffix, lbl }) => (
            <div key={lbl} className="text-center px-6 border-l border-white/[.08]">
              <div className="text-[40px] font-black text-white leading-none tracking-tighter">
                {val}{suffix && <span className="text-xl text-[#FF5F00] font-bold">{suffix}</span>}
              </div>
      ...
      {/* ── STATS CARDS ── */}
      <div className="grid grid-cols-3 gap-3 mb-7" style={{ animation: 'fadeUp .4s .08s ease both' }}>
        {['CAR','BIKE','TRUCK'].map(t => (
          <div key={t} className="bg-white border border-[#E0E0E0] rounded-sm p-6 flex items-center gap-5 hover:shadow-lg hover:-translate-y-0.5 transition-all">
            ...
            <div>
              ...
              <div className="text-[11px] font-semibold uppercase tracking-widest text-[#AAAAAA] mt-1">
                {TYPE_LABELS[t]}{vehicles.filter(v => v.type === t).length !== 1 ? 's' : ''}
              </div>
            </div>
          </div>
        ))}
      ...
      {/* ── LAYOUT ── */}
      <div className="grid gap-4 items-start" style={{ gridTemplateColumns: '360px 1fr', animation: 'fadeUp .4s .15s ease both' }}>
        ...
        <div className="bg-white border border-[#E0E0E0] rounded-sm overflow-hidden sticky top-[76px]">
          <div className="px-6 py-5 border-b border-[#E0E0E0] flex items-center justify-between">
            <span className="text-[13px] font-black uppercase tracking-widest text-[#141414]">Ajouter un véhicule</span>
          </div>
          <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-[18px]">
            ...
            {/* Type */}
            <div>
              <label className="block text-[11px] font-bold uppercase tracking-widest text-[#666] mb-1.5">Catégorie</label>
            ...
            <div>
              <label className="block text-[11px] font-bold uppercase tracking-widest text-[#666] mb-1.5">Marque & Modèle</label>
              <input name="brand" value={form.brand} onChange={handleChange} required placeholder="ex : BMW X5" className={inputCls} />
            </div>

            <div>
              <label className="block text-[11px] font-bold uppercase tracking-widest text-[#666] mb-1.5">Prix par jour (€)</label>
              <input name="basePricePerDay" type="number" min="0" step="0.01" value={form.basePricePerDay} onChange={handleChange} required placeholder="ex : 89" className={inputCls} />
            </div>

            {form.type === 'CAR' && (
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-widest text-[#666] mb-1.5">Nombre de portes</label>
                <input name="numberOfDoors" type="number" min="2" max="5" value={form.numberOfDoors} onChange={handleChange} required placeholder="ex : 4" className={inputCls} />
              </div>
            )}

            {form.type === 'BIKE' && (
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-widest text-[#666] mb-1.5">Motorisation</label>
                <div onClick={() => setForm(f => ({ ...f, electric: !f.electric }))}
                  ...
                  <div className={`w-[18px] h-[18px] border-2 rounded-sm flex items-center justify-center flex-shrink-0 transition-all ${
                    form.electric ? 'bg-[#FF5F00] border-[#FF5F00]' : 'bg-white border-[#E0E0E0]'
                  }`}>
                    {form.electric && <span className="text-white text-[11px] font-bold">✓</span>}
                  </div>
                  <span className="text-sm font-semibold text-[#1a1a1a]">Vélo électrique</span>
                </div>
              </div>
            )}
            ...
            {form.type === 'TRUCK' && (
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-widest text-[#666] mb-1.5">Charge utile (kg)</label>
                <input name="loadCapacity" type="number" min="0" value={form.loadCapacity} onChange={handleChange} required placeholder="ex : 3500" className={inputCls} />
              </div>
            )}
        ...
        <div className="bg-white border border-[#E0E0E0] rounded-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-[#E0E0E0] flex items-center justify-between">
            <span className="text-[13px] font-black uppercase tracking-widest text-[#141414]">Catalogue de véhicules</span>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#F2F2F2] border-b-2 border-[#E0E0E0]">
                {['Réf.','Type','Modèle','Tarif / jour','Détails',''].map(h => (
            ...
            <tbody>
              {vehicles.length === 0 ? (
                <tr><td colSpan={6} className="px-5 py-16 text-center">
                  ...
                  <span className="text-[13px] font-bold uppercase tracking-widest text-[#AAAAAA]">Aucun véhicule — commencez par en ajouter un</span>
                </td></tr>
              ) : vehicles.map(v => (
                <tr key={v.id} className="border-b border-[#E0E0E0] last:border-0 hover:bg-[#FFF8F5] transition-colors">
                  <td className="px-5 py-4 text-xs font-bold text-[#AAAAAA]">#{v.id}</td>
                  ...
                  <td className="px-5 py-4 text-[15px] font-bold text-[#141414]">{v.brand}</td>
                  <td className="px-5 py-4 text-[18px] font-black text-[#141414]">
                    {v.basePricePerDay}<span className="text-[13px] font-semibold text-[#FF5F00] ml-0.5">€</span>
                  </td>
                  <td className="px-5 py-4 text-xs font-semibold uppercase tracking-widest text-[#AAAAAA]">
                    {v.type === 'CAR' && `${v.numberOfDoors} portes`}
                    {v.type === 'BIKE' && (v.electric
                      ? <span className="inline-flex items-center gap-1 text-[#0095A8] bg-[#0095A8]/[.08] border border-[#0095A8]/20 px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-widest">⚡ Électrique</span>
                      : 'Mécanique')}
                    {v.type === 'TRUCK' && `${v.loadCapacity} kg`}
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <button onClick={() => setCalendarVehicle(v)}
